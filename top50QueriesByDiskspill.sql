-- This query retrieves query execution statistics from Amazon Redshift system tables over the past 7 days.
-- It provides details such as execution time, compile time, disk spill, number of executions, and alert events.
-- The query groups results by database, user, query signature (MD5 hash), and involved tables.
-- It filters out system queries and ranks queries based on disk spill and execution frequency, returning the top 50.
